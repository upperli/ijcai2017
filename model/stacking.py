...
def stack1(file_path, output_path, model_type, begin_day, end_day, expansion=False, save_model=True, 
        output=True, online=False, steps=1000, train_set="base", valid_set="base", test_set="base", cv=4, shuffle=False, weight=[50, 15, 15, 5]):
  final_lis = [] 
  for gap in range(begin_day, end_day):
    ...
    train,valid,test_online = preprocess.split_lastweek(data,comm, train_set, valid_set, test_set)
    ...
    """
    clfs = [(fitXGBModel, {"learning_rate":0.04, "n_estimators":1000, "seed":1}), 
            (fitXGBModel, {"max_depth":6, "learning_rate":0.04, "n_estimators":1000, "seed":2}),
            (fitXGBModel, {"colsample_bytree":0.6, "gamma":0.2, "learning_rate":0.01, "colsample_bylevel":0.7, 
                           "n_estimators":1000, "seed":7}), 
            (fitXGBModel, {"objective":"reg:linear", "subsample":0.7, "gamma":0.3, "learning_rate":0.04, 
                           "n_estimators":1000, "seed":3}), 
            (fitLGBModel, {"application":"fair", "metric":"fair", "feature_fraction":0.6, "learning_rate":0.02, 
                           "feature_fraction_seed":1, "bagging_seed":2}),
            (fitLGBModel, {"application":"huber", "metric":"huber", "feature_fraction":0.8, "learning_rate":0.03,
                           "feature_fraction_seed":2, "bagging_seed":1, "num_leaves":64}),
            (fitLGBModel, {"application":"regression_l1", "metric":"l1", "feature_fraction":0.6, "learning_rate":0.02,
                           "feature_fraction_seed":3, "bagging_seed":4, "lambda_l1":0.9, "max_depth":10}),
            (fitLGBModel, {"application":"regression_l2", "metric":"l2", "feature_fraction":0.6, "boosting":"gbrt",
                           "feature_fraction_seed":4, "bagging_seed":3, "lambda_l2":0.7, "nun_leaves":256, "max_bin": 240}),
            (fitRFModel, {"max_features":0.1, "max_depth":7, "random_state":1}), 
            (fitRFModel, {"max_features":"sqrt", "max_depth":6, "random_state":2}),
            (fitGBRTModel, {"loss":"lad", "learning_rate":0.01, "max_depth":6,"max_features": "sqrt", "random_state":2}),
            #(fitGBRTModel, {"loss":"ls", "learning_rate":0.02, "max_depth":8,"max_features": 0.7, "random_state":3}),
            #(fitGBRTModel, {"loss":"huber", "learning_rate":0.03, "max_depth":6,"max_features": 0.5, "random_state":4}),
         #   (fitLinearSVRModel, {}),
         #   (fitRidgeModel, {})] 
         #   (fitLRModel, {})] 
    """
    ...
    for idx, clf in enumerate(clfs):
      ...
      for i, (train, test) in enumerate(skf):
        ...
        if save_model:
          joblib.dump(reg, "%s/%d_%d_%d.m"%(file_path, gap, idx, i))
        stage2_train[test, idx] = reg.predict(X_test)
        stage2_valid_temp[:, i] = reg.predict(valid_x)
        stage2_test_temp[:, i] = reg.predict(test_x_tmp)
      stage2_valid[:,idx] = stage2_valid_temp.mean(1)
      stage2_test[:,idx] = stage2_test_temp.mean(1)
      print "Gap ", gap,  "Model %d: %.6f"%(idx, evaluate(stage2_valid_temp.mean(1), valid_y))
    ...
    if save_model:
      joblib.dump(reg1, "%s/stage2_lr.m"%(file_path))
      joblib.dump(reg3, "%s/stage2_XGB.m"%(file_path))

    if output:
      pred = reg1.predict(stage2_test)
      output_test(pred, test_online[1], gap, "LR", eva1, output_path)
      pred = reg2.predict(stage2_test)
      output_test(pred, test_online[1], gap, "Ridge", eva2, output_path)
      pred = reg3.predict(stage2_test)
      output_test(pred, test_online[1], gap, "XGB", eva3, output_path)

    print "Gap %d Start Time:"%gap, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())) 
  print "Week 1 Eval:", np.average(final_lis)

if __name__ == '__main__':
  file_path = "./Model_Save/Stacking_Version3.0"
  output_path = "./Model_Save/Stacking_Result_Version3.0"
  ...
  shuffle = True
  weight = [70,25,25,0]
  stack1(file_path, output_path, model_type, begin_day, end_day, expansion=expansion, save_model=save_model, 
      output=output, online=online, steps=steps, train_set=train_set, valid_set= valid_set, 
      test_set=test_set, cv=cv, shuffle=shuffle, weight=weight)    
"""
  for i in [A, B, C]:
    train_set = i
    valid_set = i
