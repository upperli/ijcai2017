...
def output_test(pred,test_comm,gap,t):
    ...
    path = os.path.join(config.RESULT,'xgb')
...
def fitModelAll(train_x,train_y,valid_x,valid_y,seed,gap,rounds):
    params = {
                'objective':myObjective6,
                'max_depth':7, 
                'learning_rate':0.03, 
...
def fitModel(train_x,train_y,valid_x,valid_y,seed,gap):
    ...
    pred = reg.predict(valid_x)
    valid_y_exp = valid_y
    #dic = sorted(reg._Booster.get_fscore(fmap = os.path.join(config.DATA,'%d/data.names'%gap)).items(),key = lambda x: -x[1])
    #for k,v in dic:
    #    print >> sys.stderr,"%s,%f"%(k,v)
    ''' 
    fout = open('model.txt','w')
...
def run():
    ...
    for i in xrange(7):
        ...
        for t in [1,2]:
            ...
            regs = []
            preds = []
            for seed in [1,2,3,5,6,7,8,9,11,15]:
            #for seed in [1]:
                reg,e,ep,pred = fitModel(train_x,train_y,valid_x,valid_y,seed,gap)    
                #eps += list(ep)
                preds.append(pred)
                rounds = reg.best_iteration
                reg,e,ep,pred = fitModelAll(train_x,train_y,valid_x,valid_y,seed,gap,rounds)
                regs.append(reg)
                #pred = map(lambda x: int(max(0,x)),pred)
